...
class HyperplaneHashBase {
 ...
 public:
  ...
  typedef Eigen::Matrix<CoordinateType,
                        Eigen::Dynamic,
                        Eigen::Dynamic,
  ...
  int_fast32_t get_k() const {
    return k_;
  }

  const MatrixType& get_hyperplanes() const {
    return hyperplanes_;
  }
  ...
  void hash(const VectorType& point,
    ...
    static_cast<const Derived*>(this)->get_multiplied_vector_all_tables(point,
        tmp_hash_vector);
 ...
 protected:
  HyperplaneHashBase(int dim,
                     int_fast32_t k,
                     int_fast32_t l,
                     uint_fast64_t seed) : dim_(dim), k_(k), l_(l),
                                           seed_(seed) {
    if (dim_ < 1) {
  ...
  int_fast32_t l_;
  uint_fast64_t seed_;
  Eigen::Matrix<CoordinateType,
                Eigen::Dynamic,
                Eigen::Dynamic,
                Eigen::ColMajor> hyperplanes_;
    ...
    void setup_probing(const TransformedVectorType& hash_vector,
      ...
      for (int_fast32_t ii = 0; ii < l_; ++ii) {
        ...
        for (int_fast32_t jj = 0; jj < k_; ++jj) {
          ...
          main_table_probe_[ii] = main_table_probe_[ii]
              | (hash_vector[ii * k_ + jj] >= 0.0);
        }
      }
      ...
      for (int_fast32_t ii = 0; ii < l_; ++ii) {
        ...
        std::sort(sorted_hyperplane_indices_[ii].begin(),
                  sorted_hyperplane_indices_[ii].end(),
                  comp);
      }
    ...
    bool get_next_probe(HashType* cur_probe, int_fast32_t* cur_table) {
      ...
      *cur_probe = main_table_probe_[*cur_table] ^ cur_candidate.hash_mask_;
      ...
      if (cur_candidate.last_index_ != k_ - 1) {
        ...
        CoordinateType next_score = cur_score -
        ...
        heap_.insert(next_score, ProbeCandidate(*cur_table,
              next_mask,
              cur_candidate.last_index_ + 1));

        // adding a new flipped index
        next_mask = cur_candidate.hash_mask_
          ^ (HashType(1) << (k_ - next_index - 1));
    ...
    class ProbeCandidate {
     public:
      ProbeCandidate(int_fast32_t table = 0,
                     HashType hash_mask = 0,
                     int_fast32_t last_index = 0) : table_(table),
...
class HyperplaneHashDense : public HyperplaneHashBase<
...
class HyperplaneHashSparse : public HyperplaneHashBase<
 ...
 public:
  ...
  HyperplaneHashSparse(IndexType dim,
                       int_fast32_t k,
                       int_fast32_t l,
                       uint_fast64_t seed)
      : HyperplaneHashBase<HyperplaneHashSparse<CoordinateType,
                                                HashType,
                                                IndexType>,
                           DerivedVectorT,
                           CoordinateType,
  ...
  void get_multiplied_vector_single_table(const DerivedVectorT& point,
    ...
    for (IndexType ii = 0; ii < static_cast<IndexType>(point.size()); ++ii) {
      *res += point[ii].second
          * this->hyperplanes_.col(point[ii].first).segment(l * this->k_,                                                                   this->k_);
    }
  }
};
